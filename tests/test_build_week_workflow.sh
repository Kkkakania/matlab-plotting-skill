...
for path in "$DEMO" "$BUILDER" "$VERIFIER" "$CONTRACT" "$FIXTURE" "$BENCHMARK" "$ROOT_DIR/docs/build-week-2026.md" \
...
grep -q -- "build_review_bundle.py" "$DEMO"
grep -q -- "verify_review_bundle.py" "$DEMO"
grep -q -- "candidate_manifest.json" "$CONTRACT"
grep -q -- "review_report.html" "$CONTRACT"
